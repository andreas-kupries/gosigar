...
func (self *Mem) Get() error {
	...
	cgroupLimitAsString, err := ioutil.ReadFile(SysMemd + "/memory.limit_in_bytes")
	if err != nil {
		// The limit is not available or not accessible. Keep the host data.
		return nil
	}

	cgroupLimit, err := strtoull(strings.Split(string(cgroupLimitAsString), "\n")[0])
	...
	if err := parseCgroupMeminfo(table); err != nil {
		return nil
	}
...
func parseCgroupMeminfo(table map[string]*uint64) error {
	return readFile(Procd+"/memory.stat", func(line string) bool {
		fields := strings.Split(line, " ")
		if ptr := table[fields[0]]; ptr != nil {
			num := strings.TrimLeft(fields[1], " ")
			val, err := strtoull(strings.Fields(num)[0])
